...
local function call(name, arguments)
    local status, message = pcall(emitter.emit, emitter, name, arguments)
...
local function on_char_callback(window, codepoint)
    local char = string.char(codepoint)
    entered_text = entered_text .. char
end
...
local function draw_sprite_cheap(x, y, sprite)
    ...
    engine.draw_atlas_segment(x, y, atlas_x, atlas_y, width, height)
end
...
function engine.initialize(props)
    ...
    if atlas_exists then
        ...
        utils.log("atlas.png not found. Atlas will not be loaded") 
    end
...
function engine.run()
    ...
    glfw.SetCharCallback(window, on_char_callback)
...
function engine.draw_atlas_segment(x, y, atlas_x, atlas_y, width, height)
